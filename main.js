...
//     new Car(road.getLaneCenter(2), -900, 30, 50, "DUMMY", 4, getRandomColor()),
//     new Car(road.getLaneCenter(1), -1100, 30, 50, "DUMMY", 4, getRandomColor()),
//     new Car(road.getLaneCenter(2), -1100, 30, 50, "DUMMY", 4, getRandomColor()),
// ];
const traffic = []
lanes = [0, 1, 2, 3]
number_of_cars = [0, 1, 2]
for (let i = 1; i < 100; i += 2) {
    let noc = number_of_cars[Math.floor(Math.random() * number_of_cars.length)]
    for (let j = 0; j < noc; j++) {
        let l = lanes[Math.floor(Math.random() * lanes.length)]
        const c = new Car(road.getLaneCenter(l), i*-100, 30, 50, "DUMMY", 4, getRandomColor())
        traffic.push(c)
    }
}
